...
const SkinDetectionScreen = ({ navigation }) => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const prediction = await ModelService.predictSkinDisease(selectedImage.uri);
            ...
            if (prediction.isValid) {
                ...
                const testId = await DatabaseService.saveTestResult({
                    testType: TEST_TYPES.SKIN_DISEASE,
    ...
    const handleViewHistory = () => {
        navigation.navigate('History', { testType: TEST_TYPES.SKIN_DISEASE });
    };
        ...
        <LinearGradient
            ...
            <ScrollView
                ...
                <Text style={styles.subtitle}>
                    Capture or upload a skin image for analysis
                </Text>

                <ImageCapture onImageSelected={handleImageSelected} testType="SKIN_DISEASE" />
                    ...
                    <Card variant="elevated" style={styles.resultCard}>
                        ...
                        {!result.isValid && (
                            <Card variant="glass" style={[
                                ...
                                <Text style={[
                                    ...
                                    {result.validationStage === 0 ? ' Non-Skin Image Detected' :
                                     result.validationStage === 1 ? 'ℹ Image Quality Issues' : '⚠️ Content Validation Failed'}
                                </Text>
                                <Text style={styles.warningText}>{result.validationReason}</Text>
                                <Text style={styles.warningSubtext}>
                                    {result.validationStage === 0 
                                        ? 'This appears to be a non-skin image. Please use clear skin images for analysis.'
                                        : result.validationStage === 1 
                                        ? 'Please ensure your image has proper lighting, focus, and shows the relevant medical area.'
                                        : 'This image may not contain appropriate medical content. Please use clear skin images.'
                                    }
                                </Text>
                        ...
                        <View style={styles.confidenceContainer}>
                            {result.isValid ? (
                                ...
                                <Text style={styles.nonSkinResult}>
                        ...
                        {result.isValid && (
                            <View style={styles.singlePredictionContainer}>
                                ...
                                <Text style={styles.diseaseDescription}>
                                    {getDiseaseDescription(result.topPrediction.label, 'skin')}
                                </Text>
                            </View>
                        )}
                        ...
                        {result.allPredictions.slice(0, 5).map((pred, index) => (
                            <View key={index} style={styles.predictionRow}>
                                <Text style={styles.predictionRowLabel}>{pred.label}</Text>
                                <Text style={styles.predictionRowValue}>
...
export default SkinDetectionScreen;
